...
# ROCM_MOCK=1 uvicorn main:app --reload --port 8000  (for testing without AMD hardware)
# Without ROCM_MOCK, requires rocm-smi installed and a real AMD GPU (AMD Developer Cloud container)
...
async def lifespan(app: FastAPI):
    ...
    print(f"[main] ROCM_MOCK mode: {rocm_monitor.MOCK_MODE}")
    yield
...
async def root():
    ...
    return {
        "status": "running",
        "mock_mode": rocm_monitor.MOCK_MODE,
        "training_active": training_job.is_running(),
        "injection_state": rocm_monitor.get_injection_state()
    }
...
@app.post("/api/inject-bottleneck")
async def inject_bottleneck():
    """
    Manually inject a memory bottleneck for demo purposes.
    
    This spikes memory usage on demand for reliable live demo timing
    instead of waiting for natural bottlenecks to occur.
    """
    training_job.inject_bottleneck()
    print("[main] Memory bottleneck injected via API")
    return {"status": "bottleneck_injected"}


@app.post("/api/inject-state/{state_name}")
async def inject_state(state_name: str):
    """Select a CSV-backed mock state for replay through the model pipeline."""
    allowed_states = {"healthy", "memory_bound", "comms_bound", "power_throttled"}
    if state_name not in allowed_states:
        return {"status": "error", "message": f"Unknown state: {state_name}"}

    rocm_monitor.set_injection_state(state_name)
    print(f"[main] CSV injection state set to: {state_name}")
    return {"status": "ok", "injection_state": state_name}


@app.post("/api/inject-clear")
async def clear_injection_state():
    """Clear any active CSV-backed injection state."""
    rocm_monitor.clear_injection_state()
    print("[main] CSV injection state cleared")
    return {"status": "ok", "injection_state": None}
